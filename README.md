...
Published Tech Notes are intentionally excluded from this repository. The application reads Markdown content from the directory configured by `STUDY_DIR`; uploaded public attachments, administrator-only files, comments, and private journal entries use separately configured persistent directories. Journal entries default to a `journal` directory next to `STUDY_DIR`, can be overridden with `JOURNAL_DIR`, and are available only through the authenticated administration console.
