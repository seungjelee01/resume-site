...
`RESUME_SHARE_TOKEN`은 `/resume`에 적용하고 `STUDY_SHARE_TOKEN`은 관리자가 Tech Notes를 공유 링크 방식으로 설정할 때 사용합니다. 관리자는 공개 방식을 선택하고 Google 로그인 사용자의 접근 허용 여부를 별도로 설정할 수 있습니다. 기존 AI Chat 입장 티켓은 접근 권한을 부여하지 않습니다. 두 서비스에 동일한 `INTERNAL_API_SECRET_FILE`을 설정하고 resume에는 `AI_CHAT_INTERNAL_URL=http://127.0.0.1:3100`을 설정하세요. 자세한 절차는 `ai-chat/deploy/UNIFIED_AUTH.md`를 참고하세요.
