...
app.post('/admin/new', attachmentUpload.array('attachmentFiles', 10), async (req, res, next) => { try { const slug = await savePost(req); res.redirect(303, `/admin/edit/${encodeURIComponent(slug)}`); } catch (error) { next(error); } });
app.post('/admin/edit/:slug', attachmentUpload.array('attachmentFiles', 10), async (req, res, next) => { try { const slug = await savePost(req, req.params.slug); res.redirect(303, `/admin/edit/${encodeURIComponent(slug)}`); } catch (error) { next(error); } });
