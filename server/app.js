...
app.get('/admin/', async (_req, res, next) => {
  try {
    ...
    const diskContent = disk
      ? `<dl class="resource-status disk-status"><div><dt>디스크 사용</dt><dd>${formatGigabytes(disk.used)}</dd></div><div><dt>디스크 여유</dt><dd>${formatGigabytes(disk.available)}</dd></div></dl>`
