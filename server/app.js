...
const defaultSiteSettings = Object.freeze({
  studyAccess: studyShareToken ? 'shared' : 'public',
  studyGoogleAccess: true,
});
...
async function loadSiteSettings() {
  ...
  try {
    ...
    siteSettingsCache = {
      ...
      studyGoogleAccess: typeof parsed.studyGoogleAccess === 'boolean'
        ? parsed.studyGoogleAccess
        : defaultSiteSettings.studyGoogleAccess,
    };
  } catch (error) {
...
function shareAccess({ token, cookieName, cookiePath, acceptedCookies = [], publicAccess = async () => false }) {
  return async (req, res, next) => {
    try {
      ...
      if (await publicAccess(res.locals.portalUser)) return next();
...
const requireStudyShare = shareAccess({
  ...
  publicAccess: async (portalUser) => {
    const settings = await loadSiteSettings();
    return settings.studyAccess === 'public' || Boolean(portalUser && settings.studyGoogleAccess);
  },
});
async function canAccessStudy(cookies) {
  const settings = await loadSiteSettings();
  if (settings.studyAccess === 'public') return true;
  if (settings.studyGoogleAccess && await unifiedAuth.userFromCookies(cookies)) return true;
  if (!studyShareToken) return true;
...
const adminPlaceholderPattern = /^>\s*\*\*결과 스크린샷 위치\s+—\s+`[^`]+`\*\*:[^\r\n]*(?:\r?\n|$)/gmu;

function renderMarkdown(source) {
  const publicSource = source.replace(adminPlaceholderPattern, '');
  const html = sanitizeHtml(marked.parse(publicSource), {
    allowedTags: sanitizeHtml.defaults.allowedTags.concat(['img', 'h1', 'h2']),
    allowedAttributes: { ...sanitizeHtml.defaults.allowedAttributes, img: ['src', 'alt', 'title'], span: ['class'] },
    allowedClasses: { span: ['study-image-caption'] },
...
function studySidebar(posts) {
  ...
  const databaseChildren = ['Oracle'].filter((category) => categories.has(category));
  const databaseCount = (categories.get('Database') || 0) + databaseChildren.reduce((total, category) => total + categories.get(category), 0);
  const categoryLink = (category, count, { className = '', members = [] } = {}) => `<a${className ? ` class="${className}"` : ''} href="/study/?category=${encodeURIComponent(category)}" data-sidebar-category="${escapeHtml(category)}"${members.length ? ` data-sidebar-category-members="${escapeHtml(members.join('||'))}"` : ''}><span>${escapeHtml(category)}</span><span class="sidebar-count">${count}</span></a>`;
  const databaseGroup = databaseCount ? `<div class="sidebar-category-group">${categoryLink('Database', databaseCount, { className: 'sidebar-category-parent', members: ['Database', ...databaseChildren] })}${databaseChildren.length ? `<div class="sidebar-subcategories">${databaseChildren.map((category) => categoryLink(category, categories.get(category))).join('')}</div>` : ''}</div>` : '';
  const standaloneCategories = sortedCategories.filter(([category]) => category !== 'Database' && !databaseChildren.includes(category)).map(([category, count]) => categoryLink(category, count)).join('');
...
function studyLayout({ title = '', description = '', content, posts, isHome = false }) {
  ...
  return `<!doctype html><html lang="ko"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
    ...
    ${studySidebar(posts)}<button class="sidebar-overlay" type="button" aria-label="탐색 메뉴 닫기" data-sidebar-overlay hidden></button><main class="study-main" id="study-content" tabindex="-1">${content}</main><button class="study-chat-open" type="button" aria-label="Seungje Lee에게 문의하기" aria-expanded="false" aria-controls="study-chat" data-chat-open><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 5h16v11H9l-5 4z"/><path d="M8 9h8M8 12h5"/></svg><span>문의</span><b data-chat-unread hidden>0</b></button><section class="study-chat" id="study-chat" aria-label="Seungje Lee에게 문의하기" data-chat hidden><header><div><strong>Seungje Lee에게 문의하기</strong><span data-chat-status>연결 준비 중</span></div><button type="button" aria-label="채팅 닫기" data-chat-close>×</button></header><ol aria-live="polite" data-chat-messages></ol><p class="study-chat-privacy">문의 내용과 첨부 파일은 최근 활동일로부터 90일간 보관됩니다. <a href="/privacy">자세히</a></p><form data-chat-form><div class="study-chat-compose"><label for="chat-message">메시지</label><textarea id="chat-message" maxlength="1000" rows="2" data-chat-input></textarea><div class="study-chat-upload" data-chat-upload hidden><strong data-chat-upload-name></strong><small data-chat-upload-progress></small><progress max="100" value="0" data-chat-upload-bar></progress></div></div><div class="study-chat-actions"><label class="study-chat-attach" for="chat-file" title="ZIP 또는 EXE 파일 첨부">＋<span class="sr-only">ZIP 또는 EXE 파일 첨부</span><input id="chat-file" type="file" accept=".zip,.exe,application/zip,application/vnd.microsoft.portable-executable,application/x-msdownload" data-chat-file></label><button type="submit" data-chat-submit>전송</button></div></form></section><dialog class="study-settings" data-study-settings><form method="dialog"><div class="study-settings-title"><h2>설정</h2><button type="submit" aria-label="설정 닫기">×</button></div><label>테마<select data-study-theme><option value="system">시스템 설정</option><option value="light">라이트</option><option value="dark">다크</option></select></label><button class="study-settings-done" type="submit">완료</button></form></dialog></body></html>`;
}

app.get('/study/chat/session/', (req, res, next) => chatService.session(req, res).catch(next));

app.post('/study/chat/uploads/', express.json({ limit: '2kb' }), async (req, res) => {
  try { res.status(201).json(await chatService.beginVisitorUpload(req.get('Cookie'), req.body)); }
  catch (error) { res.status(400).json({ error: error.message }); }
});
app.put('/study/chat/uploads/:uploadId/', express.raw({ type: 'application/octet-stream', limit: '64mb' }), async (req, res) => {
  try { res.json(await chatService.appendVisitorUpload(req.get('Cookie'), req.params.uploadId, Number(req.get('Upload-Offset')), req.body)); }
  catch (error) { res.status(400).json({ error: error.message }); }
});
app.post('/study/chat/uploads/:uploadId/complete/', async (req, res) => {
  try { res.status(201).json({ message: await chatService.completeVisitorUpload(req.get('Cookie'), req.params.uploadId) }); }
  catch (error) { res.status(400).json({ error: error.message }); }
});
app.delete('/study/chat/uploads/:uploadId/', async (req, res) => {
  try { res.sendStatus(await chatService.abortVisitorUpload(req.get('Cookie'), req.params.uploadId) ? 204 : 404); }
  catch (error) { res.status(400).json({ error: error.message }); }
});
...
app.get('/study/quiz/', async (req, res, next) => {
  try {
    ...
    const content = `<link rel="stylesheet" href="/study/assets/quiz.css?v=20260902-1"><header class="study-home-header study-quiz-header"><p class="study-eyebrow">ACTIVE RECALL</p><h1>용어 퀴즈</h1><p>설명을 읽고 알맞은 키워드를 입력해 학습한 내용을 복습합니다.</p></header>${quizBody}<script src="/study/assets/quiz.js?v=20260902-3" defer></script>`;
...
app.use('/admin', (_req, _res, next) => { loadSiteSettings().then(() => next(), next); });
...
function adminLayout(title, content, email, activeSection = '') {
  ...
  const navItem = (section, href, label) => {
    ...
    return `${item}<a${activeSection === 'chats' ? ' class="is-active" aria-current="page"' : ''} href="/admin/chats/">${icons.chats}<span>실시간 문의</span><b class="admin-nav-unread" data-admin-chat-unread hidden>0</b></a>`;
  };
...
app.post('/admin/settings/study-access', async (req, res, next) => {
  try {
    ...
    const googleAccessValues = Array.isArray(req.body.studyGoogleAccess)
      ? req.body.studyGoogleAccess
      : [req.body.studyGoogleAccess];
    const studyGoogleAccess = googleAccessValues.includes('true');
    ...
    await saveSiteSettings({ studyAccess, studyGoogleAccess });
